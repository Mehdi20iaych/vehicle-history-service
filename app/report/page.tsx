...
export default function ReportPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/report', { cache: 'no-store', signal: controller.signal }).then(async response => {
      ...
      setReport(result)
      if (!purchaseTracked.current && result.price && result.price !== '0.00') {
  ...
  return <main className={styles.page}>
    ...
    {report && <>{!loading && !hasRecords && <aside className={styles.empty}><h2>No history records available</h2><p>Your access was confirmed, but the sources returned no usable history. This is not a completed history report.</p><p><strong>Do not pay again.</strong> Retry below.</p></aside>}<p>Requested VIN: <strong>{report.vin}</strong></p><p>{report.orderId.startsWith('COUPON-') ? 'Coupon access' : 'PayPal order'}: {report.orderId}</p><p>Report access lasts 24 hours in this browser. Save or print a copy for your records.</p>
