...
type PayPalActions = { order: { create: () => Promise<string> } }
type PayPalButton = { render: (element: HTMLElement) => Promise<void>; isEligible: () => boolean }
type PayPalSdk = {
...
export function preloadPayPalCheckout() {
  ...
  })().catch((error) => { paypalSdkPromise = null; throw error })
...
export function PayPalCardCheckout({ vin, email, price, quoteId, onComplete, onError }: Props) {
  ...
  const [status, setStatus] = useState('Loading PayPal and card options…')
  ...
  useEffect(() => {
    ...
    async function loadSdk() {
      ...
      const sharedOptions = {
        createOrder: async () => { setStatus('Creating your secure order…'); return createOrder() },
