...
import { analyticsCookie, recordEvent, saveReportQuote, visitorIdFromRequest } from '@/lib/site-analytics'
...
export async function POST(request: Request) {
  ...
  const quoteId = available ? await saveReportQuote({ vin, price: quote.price, sections, recordCount: quote.recordCount, sourceCount: quote.sourceCount }) : null
  ...
  const response = NextResponse.json({ available, price: available ? quote.price : null, quoteId, currency: 'USD', sections: sections.map(({ title, status, records }) => ({ title, status, count: records.length })) }, { headers: { 'Cache-Control': 'no-store' } })
