...
async function captureOrder(orderId: string) {
  ...
  let order = await paypalRequest<PayPalOrder>(
    `/v2/checkout/orders/${encodeURIComponent(orderId)}`,
  )

  if (order.id !== orderId || order.purchase_units?.length !== 1 || order.purchase_units[0].reference_id !== session.vin || !hasExpectedAmount(order, expectedPrice)) {
    throw new Error('PayPal order amount or currency did not match.')
  }

  if (order.status !== 'COMPLETED') {
    if (!(await availableRecords(session.vin)).available) throw new Error('Records are currently unavailable. Payment has not been captured. Please try again later.')
    if (order.status !== 'APPROVED') {
      throw new Error(`PayPal order is not ready to capture (status: ${order.status || 'unknown'}).`)
    }
...
export async function GET(request: Request) {
  ...
  try {
    ...
    await recordEvent(request, 'purchase', { visitorId: visitorIdFromRequest(request), orderId, amount: session.price, currency: PAYPAL_CURRENCY })
